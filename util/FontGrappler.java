...
public class FontGrappler {
  ...
  static void usage() {
    ...
    System.out.println("  -img         - just capture an image of given width and height");
  }
  ...
  static void outputImg() {
    int iw = img.getWidth();
    int ih = img.getHeight();
    if (cw + ox > iw) {
      cw = iw - ox;
      System.out.println("warning: width changed to " + cw);
    }
    if (ch + oy > ih) {
      ch = ih - oy;
      System.out.println("warning: height changed to " + ch);
    }
    out("/* Image generated by FontGrappler\n");
    out(" * from file " + file + "\n");
    out(" * image size: " + cw + "x" + ch + "\n");
    out(" */\n\n");
    
    out("unsigned const char const img_" + new File(file).getName().toLowerCase().replace('.', '_') + "[" + (ch*cw) + "] = {\n");
    
    
    for (int y = oy; y < oy+ch; y++) {
      for (int x = ox; x < ox+cw; x += 8) {
        int g = 0;
        for (int cx = x; cx < x + 8; cx++) {
           if ((img.getRGB(cx, y) & 0xffff00) != 0x0) {
             g |= (1<<(8-(cx-x)-1));
           }
        }
        out("0x");
